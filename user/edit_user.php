...
include ('user_header.php');
   $db = mysqli_connect('localhost', 'root', '', 'fct_library');   
                $user_query = mysqli_query($db, "SELECT * FROM users WHERE user_id='$id_session'")or die(mysqli_error());
...
<?php
    $db = mysqli_connect('localhost', 'root', '', 'fct_library'); 
    $query=mysqli_query($db,"SELECT  * from users where user_id='$id_session'") or die(mysqli_error());
                            ...
                            <form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-4">
										...
										<img src="<?php echo $row['user_image']; ?>" width="100px" height="100px" style="border:4px groove #CCCCCC; border-radius:5px;">
										<?php else: ?>
                                ...
                                <div class="form-group">
                                    <label class="control-label col-md-4" for="last-name">SchoolID
                                    </label>
                                    <div class="col-md-4">
                                        <input type="text" value="<?php echo $row['school_id']; ?>" name="school_id" id="last-name2" required="required" class="form-control col-md-7 col-xs-12">
                                    </div>
                                </div>
                                <div class="form-group">
...
{
echo "<script>alert('Password do not match!'); window.location='user_profile.php'</script>";
}else
{
mysqli_query($db," UPDATE users SET firstname='$firstname' lastname='$lastname', school_id='$school_id', password='$password', 
confirm_password='$confirm_password', user_image='$still_profile' WHERE user_id = '$id' ")or die(mysqli_error());
echo "<script>alert('Successfully Update User Info!'); window.location='user_profile.php'</script>";	
}
									}
                                    else{
...
$result = mysqli_query($db,"select * from users where user_type= 'student'") or die (mysqli_error());
...
{		
mysqli_query($db,"UPDATE users SET firstname='$firstname', lastname='$lastname', school_id='$school_id', password='$password', 
confirm_password='$confirm_password', user_image='$profile' WHERE  user_id = '$id' ")or die(mysqli_error());
